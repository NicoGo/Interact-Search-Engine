...
class AcceuilController
{
	...
	public function search($keyword)
	{
		if(strlen($keyword)>1)
		{
			...
			$sites = Sites::Where("url_dev","like","%$keyword%")->orWhere("server_name","like","%$keyword%")->join('map_user_sites', 'sites.id', '=', 'map_user_sites.id_site')->orderBy("favorite","DESC")->orderBy("views","DESC")->get(['map_user_sites.id AS mus_id', 'sites.*']);
